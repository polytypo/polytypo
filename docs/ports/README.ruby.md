...
**Status: planned, not yet started.** This document is the specification of the port's public surface, kept in the spec repository so the API is agreed before any code exists. No gem is published.

Spec version: **1.0.0** · locales: **10** · rules: **9** · not yet on RubyGems.
...
**Planned API — not installable.** No gem exists yet; the command below is what it will be once the port ships, not something you can run today.
