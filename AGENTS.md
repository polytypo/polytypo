...
from spec data via the published `polytypo` npm package, and renders `CHANGELOG.md` and the
`/changelog` page from `brand/tools/changelog.json` — a released spec version gets its entry there
before its `spec-v*` tag is pushed, and unreleased versions get none. Documentation is in `docs/`; read
